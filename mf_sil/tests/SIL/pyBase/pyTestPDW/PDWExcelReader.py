...
# Constants
#********************************************************************
PDW_PARAM_NAME_ROW       = 1 # first row with parameter names
PDW_FIRST_TEST_ROW       = 2 # first Scenario Test row in sheet Situations
PDW_FIRST_BTN_ROW        = 2 # first Button row in sheet HMI
PDW_FIRST_VHCL_ROW       = 2 # first Vehicle row in sheet Vehicles
PDW_FIRST_STRFURN_ROW    = 2 # first Furniture row in sheet StreetFurniture
PDW_FIRST_PARAM_ROW      = 2 # first Parameter row in sheet Parameters
...
pdwParamConfigType = namedtuple("pdwParamConfigType", "value factor")

class PDWExcelReader:
    # Class constructor
    def __init__(self):
        ...
        self.Parameters = {}
        colKey = self.getColNumByName("Parameter name", self.sheetForParameters, PDW_PARAM_NAME_ROW)
        colFactor = self.getColNumByName("Desired factor", self.sheetForParameters, PDW_PARAM_NAME_ROW)
        colValue = self.getColNumByName("Parameter value - DO NOT FILL", self.sheetForParameters, PDW_PARAM_NAME_ROW)
        for currRow in range(PDW_FIRST_PARAM_ROW, self.sheetForParameters.max_row + 1):
            ...
            self.Parameters[key] = pdwParamConfigType(value, factor)          
        ...
        self.firstTestRow = PDW_FIRST_TEST_ROW
        ...
        for paramName in paramConfig:
            colParam = self.getColNumByName(paramName, self.sheetForSituations, PDW_PARAM_NAME_ROW)
        ...
        self.HMIButtons = {}
        colKey = self.getColNumByName("Button", self.sheetForHMI, PDW_PARAM_NAME_ROW)
        colValue = self.getColNumByName("Value", self.sheetForHMI, PDW_PARAM_NAME_ROW)
        for currRow in range(PDW_FIRST_BTN_ROW, self.sheetForHMI.max_row + 1):
        ...
        self.Vehicles = {}
        colKey = self.getColNumByName("Model", self.sheetForVehicles, PDW_PARAM_NAME_ROW)
        colLength = self.getColNumByName("Length", self.sheetForVehicles, PDW_PARAM_NAME_ROW)
        colWidth = self.getColNumByName("Width", self.sheetForVehicles, PDW_PARAM_NAME_ROW)
        colHeight = self.getColNumByName("Height", self.sheetForVehicles, PDW_PARAM_NAME_ROW)
        colMovieGeometry = self.getColNumByName("Movie geometry", self.sheetForVehicles, PDW_PARAM_NAME_ROW)
        for currRow in range(PDW_FIRST_VHCL_ROW, self.sheetForVehicles.max_row + 1):
        ...
        self.StrFurniture = {}
        colKey = self.getColNumByName("Type", self.sheetForStreetFurniture, PDW_PARAM_NAME_ROW)
        colLength = self.getColNumByName("Length", self.sheetForStreetFurniture, PDW_PARAM_NAME_ROW)
        colWidth = self.getColNumByName("Width", self.sheetForStreetFurniture, PDW_PARAM_NAME_ROW)
        colHeight = self.getColNumByName("Height", self.sheetForStreetFurniture, PDW_PARAM_NAME_ROW)
        colMovieGeometry = self.getColNumByName("Movie geometry", self.sheetForStreetFurniture, PDW_PARAM_NAME_ROW)
        for currRow in range(PDW_FIRST_STRFURN_ROW, self.sheetForStreetFurniture.max_row + 1):
    ...
    # Use with PDW TestCatalog (TestRun excel file)
    ...
    def existsScenario(self, currRow):
        exists = False
        colCategory = self.getColNumByName("Category", self.sheetForSituations, PDW_PARAM_NAME_ROW)
        if colCategory != -1:
            if self.sheetForSituations.cell(row = currRow, column = colCategory).value != None: # the row contains a test situation
                exists = True
        else:
    ...
    def toBeGenerated(self, currRow):
        toBeGen = False
        colGen = self.getColNumByName("Generate Testcase", self.sheetForSituations, PDW_PARAM_NAME_ROW)
        if colGen != -1:
