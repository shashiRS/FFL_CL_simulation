...
import PDWExcelReader, PDWTestCase
...
cmTestRunConfig = {                      #      testRunName          testRunFolder
    "Perpendicular"   : cmTestRunConfigType("PDWSim_TestRun_", PDWTESTRUN_SUBFOLDER),
    "Parallel"        : cmTestRunConfigType("PDWSim_TestRun_", PDWTESTRUN_SUBFOLDER),
    "Angled"          : cmTestRunConfigType("PDWSim_TestRun_", PDWTESTRUN_SUBFOLDER),
    "Hill"            : cmTestRunConfigType("PDWSim_TestRun_", PDWTESTRUN_SUBFOLDER),
    "Downhill"        : cmTestRunConfigType("PDWSim_TestRun_", PDWTESTRUN_SUBFOLDER)
    }
...
def runCarMakerScenario(situationID, testFileName, testRunPath):
    ...
    testRunner   = cCMEvalTestRunner(CM_PRJ_PATH, PDW_QUANTITIES, numberOfCarMakerInstances = 1, sampleRate = '10ms')
...
def performTestSession():
    ...
    xlsReader = PDWExcelReader.PDWExcelReader()
    ...
    #for row in range(42, 47):
        ...
        if not cat in list(cmTestRunConfig.keys()): # unknown test category
            print("Test scenario {} not generated: unknown category {}".format(scenario["Situation ID"], cat))
            continue

        testFolderName = os.path.join(PDWGENERATED_TEMP_PATH, cmTestRunConfig[cat].testRunFolder) 
        testFileName = "{}{}".format(cmTestRunConfig[cat].testRunName, scenario["Situation ID"])
        testRunPath = os.path.join(testFolderName, testFileName)

        if doRegen:
            # generate CarMaker test scenario
            TCasePDW = PDWTestCase.PDWTestCase(scenario, xlsReader)
            PDWTestRun = cCMTestRun(name=testFileName)
            # PDWTestRun.Road.Definition.DigRoad = cmTestRunConfig[cat].roadName
            TCasePDW.PDWTestRunFactory(PDWTestRun)
            PDWTestRun.save(testFolderName)
    ...
    try:  
        ...
        if len(args) == 2:
            ...
            if args[1] == "-eval":
                ...
                print("PDW test catalogue:           {}".format(TESTCATALOGUE_PATH))
                print("Generated CarMaker scenarios: {}".format(PDWGENERATED_PATH))
            elif args[1] == "-full":
                doRegen = True
                doRun = True
                doEval = True
                ...
                print("Unknown argument for PDWTestSession.py")
                sys.exit(0)
            performTestSession()
            print("All done. Execution time: {}\n".format(calcExecTime(startTime)))
        else:
            print("Unexpected number of arguments for PDWTestSession.py")

    except Exception as e:
        print("Exception = {}".format(e))
        input("Exception in executing PDWTestSession.py.\nHit enter To Close...")
