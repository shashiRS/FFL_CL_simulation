...
def __main__():
    if(checkIfPathNotExist(WHPGENERATED_TEMP_PATH)):
        print("Temp path with generated testruns does not exist: {}".format(WHPGENERATED_TEMP_PATH))
        sys.exit()
    if(checkIfPathNotExist(WHPGENERATED_PATH)):
        print("Final path for generated testruns does not exist: {}".format(WHPGENERATED_TEMP_PATH))
        sys.exit()
    
    #remove read only flag
    os.chmod(WHPGENERATED_PATH, stat.S_IWRITE )

    tempTestRuns_list = []
    finalTestRuns_list = []

    #save in a list the testruns from the temp path
    tempTestRuns_list = saveTestsNameInList(WHPGENERATED_TEMP_PATH)
    #save in a list the testruns from final path
    finalTestRuns_list = saveTestsNameInList(WHPGENERATED_PATH)
    
    #delete TestRuns from final path that were not regenerated this time
    for fTest in finalTestRuns_list:
        if not checkIfFileExist(WHPGENERATED_TEMP_PATH, fTest):
            try:
                os.remove(WHPGENERATED_PATH + fTest)
            except:
                print("Cannot remove file {} from {}".format(fTest, WHPGENERATED_PATH))
    ...
    for fTest in tempTestRuns_list:
        ...
        if checkIfFileExist(WHPGENERATED_PATH, fTest):
            #save in a string all testRun text without the Date (line 3)
            newStringTestRun = saveTestRunInString(WHPGENERATED_TEMP_PATH, fTest)
            oldStringTestRun = saveTestRunInString(WHPGENERATED_PATH, fTest)
            ...
            copy = True
        if copy == True:
            copyTestRun(WHPGENERATED_TEMP_PATH, WHPGENERATED_PATH, fTest)
