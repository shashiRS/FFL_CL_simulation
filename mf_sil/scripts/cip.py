...
class NetworkAccess(abc.ABC):
    ...
    def is_online(self) -> bool:
        """Tell if interface is able to work online"""
...
class NoNetworkAccess(NetworkAccess):
    """Implementation of the network access interface without any access in offline mode"""
    def read(self, url: str) -> bytes:
        raise URLError('Cannot connect to network in offline mode')
...
def calculate_sha256_for_url(url: str, network_access: NetworkAccess) -> str:
    """Calculate the SHA256 checksum for the resource under the given url.
       ...
       The function strips out all end-of-line characters to avoid differences
...
def get_cip_config_dir() -> pathlib.Path:
    # use environment variable if that is available
    config_dir = os.environ.get(
        'CIP_USER_CONFIG_DIR',
        str(pathlib.Path.home() / "cip_config_dir")
    )
...
class BootstrapInfoFactory:
    ...
    DEFAULT_BOOTSTRAP_INFO_PATH = get_bootstrap_config_dir() / "bootstrap.json"

    def __init__(self, network_access: NetworkAccess = None,
                 central_configuration_endpoint: str = None,
                 bootstrap_info_path: pathlib.Path = None):
        if network_access:
            self.__network_access = network_access
        else:
        ...
        if bootstrap_info_path:
            self.__boostrap_info_path = bootstrap_info_path
        else:
            self.__boostrap_info_path = self.DEFAULT_BOOTSTRAP_INFO_PATH

    def __does_local_bootstrap_info_file_exist(self, filesystem_access: FilesystemAccess):
        """Check if bootstrap config file exists locally"""
        return filesystem_access.exists(self.__boostrap_info_path)

    def is_local_bootstrap_info_file_up_to_date(self, bootstrap_local_config,
                                     filesystem_access: FilesystemAccess) -> bool:
        return self.__does_local_bootstrap_info_file_exist(filesystem_access)\
               and (time.time() - filesystem_access.getmtime(self.__boostrap_info_path)
    ...
    def create_from_file(self, bootstrap_local_config, filesystem_access: FilesystemAccess) -> BootstrapInfo:
        if not self.__network_access.is_online() and self.__does_local_bootstrap_info_file_exist(filesystem_access):
            print('Offline mode. Reusing configuration in: ' + str(self.__boostrap_info_path))
        elif not self.is_local_bootstrap_info_file_up_to_date(bootstrap_local_config, filesystem_access):
            try:
                success = download_file(self.__central_configuration_endpoint, self.__boostrap_info_path, True,
                          self.__network_access, filesystem_access)
                if not success:
                    print('Failed to download bootstrap configuration from server')
            except URLError as download_error:
                raise FileNotFoundError('Unable to find bootstrap configuration in: ' + str(self.__boostrap_info_path)) from download_error

        json_bytes = filesystem_access.read(self.__boostrap_info_path)
...
def update_file_if_needed(path: pathlib.Path,
    ...
    if not network_access.is_online():
        if not filesystem_access.exists(path):
            raise FileNotFoundError('Bootstrap in offline mode, but file {path} is not available'.format(path=str(path)))
        print("\nBootstrap in offline mode. Will try to proceed with the existing {path}".format(path=str(path)))
        return
...
def main(args) -> int:
    ...
    parser.add_argument(
        ...
        default=False,
        action='store_true',
        help='Do not perform any online operations'
    )
    ...
    if parsed_args.offline:
        network_access = NoNetworkAccess()
    else:
        network_access = HttpNetworkAccess()

    bootstrap_session = BootstrapSession(network_access=network_access)
    bootstrap_info = bootstrap_session.bootstrap_info
    bootstrap_py_file_info = bootstrap_info.bootstrap_files.bootstrap_py

    update_file_if_needed(path=boostrap_script,
        bootstrap_file_info=bootstrap_py_file_info,
        dont_fail_on_download_error_if_file_already_exists=True,
        suppress_message_if_update_not_needed=True,
