...
static void
MyPreFunc(tCarMVPreIF *IF)
{
	if (SimCore.State != SCState_Simulate) return;

	/*
	 if (ExtInp_File_Enabled) {
		if  (ExtInp_IF.User1.Enabled) {
			UserOut[0] = ExtInp_IF.User1.Value;
		}
	*/
	...
	*/
	...
	if (ExtInp_File_Enabled) {
		if (ExtInp_IF.User4.Enabled) {
			IF->v_0[0] = ExtInp_IF.User4.Value;
		}

		if (ExtInp_IF.User9.Enabled) {
			IF->v_0[1] = ExtInp_IF.User9.Value;
		}
	}
	else {
		IF->v_0[0] = 0;
	...
	if (ExtInp_File_Enabled) {
		if (ExtInp_IF.User1.Enabled) {
			IF->t_0[0] = ExtInp_IF.User1.Value;
		}

		if (ExtInp_IF.User2.Enabled) {
			IF->t_0[1] = ExtInp_IF.User2.Value;
		}
	}
	else {
		IF->t_0[0] = 0;
	...
	if (ExtInp_File_Enabled) {
		if (ExtInp_IF.User3.Enabled) {
			IF->r_zyx[2] = ExtInp_IF.User3.Value;
		}
	}
	else
	{
	...
	if (ExtInp_File_Enabled) {
		if (ExtInp_IF.User3.Enabled) {
			IF->rv_zyx[2] = ExtInp_IF.User10.Value;
		}
	}
	else
	{
...
MyMidFunc()
{
	if (SimpleVhclDyn == 1) {
		if (ExtInp_File_Enabled) {
			if (ExtInp_IF.User5.Enabled) {
				Car.Susp[0].r_zxy[2] = ExtInp_IF.User5.Value;
				Vehicle.FL.r_zxy[2] = ExtInp_IF.User5.Value;
			}

			if (ExtInp_IF.User6.Enabled) {
				Car.Susp[1].r_zxy[2] = ExtInp_IF.User6.Value;
				Vehicle.FR.r_zxy[2] = ExtInp_IF.User6.Value;
			}

			if (ExtInp_IF.User7.Enabled) {
				Vehicle.FL.rotv = ExtInp_IF.User7.Value;
			}

			if (ExtInp_IF.User8.Enabled) {
				Vehicle.FR.rotv = ExtInp_IF.User8.Value;
			}
		}
	}
}
...
User_Register (void)
{
	...
	if (SimpleVhclDyn == 1) {
		Log("PRE-SIM: SimpleVhclDyn activated!\nATTENTION: Please change parameter and Start&Connect again!\n");
		Set_UserCarMVPreFunc(MyPreFunc);
		Set_UserCarMVPostFunc(MyPostFunc);
	}
...
User_Param_Get (void)
{
	...
	sprintf(pre, "SimpleVhclDyn");
	SimpleVhclDyn = iGetIntOpt(ContiUSS_Parameterset_Info, pre, 0);
