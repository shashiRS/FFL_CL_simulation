	...
	if (SimCore.State >= SCState_Start && USonicRSICount) {
		...
		for (int i = 0; i < USonicRSICount; i++) {
			...
			for (int j = 0; j < ContiUSSParam.nEchoesMax[i]; j++) {
				...
				ContiUSS[i].ContiUSSEcho[j].MeasTag = 0;
				ContiUSS[i].ContiUSSEcho[j].SyncCntCurTag = 0;
				...
				sprintf(sbuf, "ContiUSS.%s.Echo.%01d.MeasTag", SensName, j);
				DDefInt(NULL, sbuf, "", &ContiUSS[i].ContiUSSEcho[j].MeasTag, DVA_None);
				sprintf(sbuf, "ContiUSS.%s.Echo.%01d.SyncCntCurTag", SensName, j);
				DDefInt(NULL, sbuf, "", &ContiUSS[i].ContiUSSEcho[j].SyncCntCurTag, DVA_None);
...
GetCurrentEchoes(int i)
{
	...
	struct tmpSensData {
		...
		int		MeasTag[_NO_OF_ECHOES_MAX_] = { 0 };
		int		SyncCntCurTag[_NO_OF_ECHOES_MAX_] = { 0 };
	} tmpSensData;
	...
	for (int k = 0; k < _NO_OF_ECHOES_MAX_; k++) {
		...
		if ( (ContiUSS[i].ContiUSSEcho[k].ContiTimeOF != 0) && (_localvar.DataTrans_Running[i][k] == 0) ) {
			...
			ContiUSS[i].ContiUSSEcho[k].MeasTag = 0;
			ContiUSS[i].ContiUSSEcho[k].SyncCntCurTag = 0;
		}
		...
		/* Evaluation of SFSP & ToF if overlapping this SFSP over 2ms Deaftime */
			...
			ContiUSS[i].ContiUSSEcho[k].ContiBurstTime = 0;
			tmpSensData.MeasTag[t] = ContiUSS[i].ContiUSSEcho[k].MeasTag;
			ContiUSS[i].ContiUSSEcho[k].MeasTag = 0;
			tmpSensData.SyncCntCurTag[t] = ContiUSS[i].ContiUSSEcho[k].SyncCntCurTag;
			ContiUSS[i].ContiUSSEcho[k].SyncCntCurTag = 0;
	...
	for (int k = 0; k < _NO_OF_ECHOES_MAX_; k++) {
		...
		if ( ( ((*ContiUSS[i].ContiUSSEcho[k].TimeOF * 1000.0) <= tmpSensData.ToF[t]) || (tmpSensData.ToF[t] == 0) )
			...
			ContiUSS[i].ContiUSSEcho[g].MeasTag = _localvar.byte_counter;
			ContiUSS[i].ContiUSSEcho[g].SyncCntCurTag= -1;
			...
			ContiUSS[i].ContiUSSEcho[g].MeasTag = tmpSensData.MeasTag[t];
			ContiUSS[i].ContiUSSEcho[g].SyncCntCurTag = tmpSensData.SyncCntCurTag[t];
			...
			ContiUSS[i].ContiUSSEcho[g].MeasTag = 0;
			ContiUSS[i].ContiUSSEcho[g].SyncCntCurTag = 0;
		...
		for (int k = 0; k < _NO_OF_ECHOES_MAX_; k++) {
			...
			if ( (_localvar.DataTrans_Running[i][k] == 1) || (j == i) ) {
				...
				if ( (ContiUSS[i].ContiUSSEcho[k].ContiTimeOF > _localvar.ToF_Counter[i][k])
					...
					// Hier kann man jetzt die Quantities mit den aktuellsten ToFs befüllen, da diese Agelaufen sind
					// Die Time-Tags sollten dann aber entsprechend vorher schon befüllt werden
					ContiUSS[i].ContiUSSEcho[k].SyncCntCurTag = _localvar.byte_counter;
...
ContiUSS_MainCalc ()
{
	...
	if (SimCore.State == SCState_Simulate) {
		...
		/* Incr. counter MeasTag & SyncCntCurTag */
		if (_localvar.byte_counter < 255)
			_localvar.byte_counter++;
		else
			_localvar.byte_counter = 0;
