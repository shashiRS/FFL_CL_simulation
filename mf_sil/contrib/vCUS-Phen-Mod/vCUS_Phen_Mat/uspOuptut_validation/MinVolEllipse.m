function [hOut, polyin] = MinVolEllipse(P, tolerance, plotResol, toId)
...
% M: the number of points on the ellipse to plot
%
% outputs:
%---------
...
%get data for ellipse plot
tq              = linspace(-pi,pi,plotResol); % for display purposes, M is the number of points on the ellipse
A               = inv(A);
[Ve,De]         = eig(A);
De              = sqrt(diag(De));
[l1,Ie]         = max(De);
...
xPlotEllipse    = U(1,:)+c(1);
yPlotEllipse    = U(2,:)+c(2);
hOut            = figure('Name',['Ellipse fit to TO ' toId]);

%show ellipse 
hold on 
plot(P(1,:), P(2,:), '.r');
plot(xPlotEllipse,yPlotEllipse, '-b');
hold off

%clean & get shape
plotEllipse         = [xPlotEllipse', yPlotEllipse'];
[ ~, indShape, ~]	= unique(plotEllipse, 'rows');
uniqueShape         = plotEllipse(sort(indShape), :);
%warning('off','MATLAB:polyshape:repairedBySimplify');
polyin              = polyshape({uniqueShape(:,1)},{uniqueShape(:,2)});
