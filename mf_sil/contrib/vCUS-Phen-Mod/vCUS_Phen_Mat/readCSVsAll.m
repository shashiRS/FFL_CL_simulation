...
for iFile = 1:nFiles
    ...
    allAmplitude = [allAmplitude; tempTable.UsRaw_amplitude_nu_u8]; %#ok<AGROW>
end %for iFile

fclose(logFid);
delete(fWb);
amplitudeMatx = [allDataInFld(:, 1), allAmplitude];
amplitudeMatx = sortrows(amplitudeMatx);
amplitudeMatx(allDataInFld(:,4) >= 127, :) = [];
%sort on first column (on timestamps)
allDataInFld = sortrows(allDataInFld);

%add some noise to data -- testing only
%allDataInFld(:,1) = awgn(allDataInFld(:,1),100, 'measured');

%initial Tx position
burstTxId   = allDataInFld(allDataInFld(:,4) >= 127, 3);
burstTxPos  = allDataInFld(allDataInFld(:,4) >= 127, 7:9);
sensIds     = sort(unique(burstTxId));
%find the first time each sensor is bursting
...
%calculate sensor to np direction
vect_uss2np     = matxData(:, 14:15) -  matxData(:, 11:12);%NP position - Rx pos in 2d
rx2np_direction = rad2deg(angle(vect_uss2np(:,1)+1i*vect_uss2np(:,2)));
newMatData      = [matxData, recalcToF, filesCell, rowInFile, rx2np_direction];
tableHdr        = ["usRaw_ToF", "rx_Id", "tx_Id", "vCUS_ToF", ...
                    "usRaw_vCus_diff", "isMatch", "TO", "Tx_x", "Tx_y", "Tx_z", ...
                    "Rx_x", "Rx_y", "Rx_z", "NP_x", "NP_y", "NP_z",...
                    "BackwardsMatchBurstNo", "TofFromLof", ...
                    "fileNames", "rowInFile", "Rx2NP_direction"];
...
[hFigSubpTOs, hAllTOs3D, hAllTOs2D, ToColors ]    = plotAllTOs(dataTO, sensPos, startTxPos);       

%to estimate centroid of ego vehicle function incomplete - use timestamps
%with at least 3 sensor positions
[ussTocentroid]               = egoCentroid(startTxPos, sensIds);

%calculate ego data- current function is incomplete use centroid
%estimations from above
%[egoDirection, egoVelocity, egoPosition]= 
getEgoData(sumTable, ussTocentroid, tStamp_us_u64);

%do direction statistics
%what is the usp
directionMatx = str2double([sumTable.TO, sumTable.rx_Id, sumTable.Rx2NP_direction]);
[directionFigH] = doDirectionStats(directionMatx, ToColors);
close(directionFigH);

%do amplitude stats
amplitudeMatx = [amplitudeMatx, sumTable.TO, sumTable.rx_Id, sumTable.tx_Id, LoF];
[ampFigH] = doAmplitudeStats(amplitudeMatx, ToColors);
close(ampFigH);
...
%==========================================================================
function [figHsubP, hFigAllTOs, h2dAllTOs, randomColors] = plotAllTOs(dataMatx, xyzSenso, startTxCord)
...
%==========================================================================
function [ussTocentroid] = egoCentroid(contour, sensID)
%get coordinates of the centroid of the polygon described by the sensors
%positions (hopefully all 12)
polyin          = polyshape({contour(:,1)},{contour(:,2)});
[x,y]           = centroid(polyin) ;
zCentro         = mean(contour(:,3));
xyzEgoCentroid	= [x,y,zCentro];

%get distances USS to center 
ussCoord        = contour(1:end-1, :);
uss2centerDist  = sqrt(sum((ussCoord - xyzEgoCentroid) .^ 2, 2));


relativeCoord   = [sensID, uss2centerDist];
tableHdr        = ["ussID", "distance"];
%make a table
ussTocentroid  = array2table(relativeCoord,"VariableNames",tableHdr);
...
%==========================================================================
%[egoDir, egoVelo, egoPos]=
function  getEgoData(dataTbl, relUssPositions, timeSt_us)
%calculates ego center position by triangulating it from sensor positions
%Work In Progress

%determine data: velo, direction every 0.5s
tStep = 0.05;
%determine data: velo, direction every 1m;
dStep = 0.5;
%dataTbl header
% tableHdr   = ["usRaw_ToF", "rx_Id", "tx_Id", "vCUS_ToF", ...
%                "usRaw_vCus_diff", "isMatch", "TO", "Tx_x", "Tx_y", "Tx_z", ...
...
%relUssPositions header
%tableHdr        = ["ussID", "xRelative", "yRelative", "zRelative"];
rx_id       = dataTbl.rx_Id;
timeStNow   = timeSt_us/1000000;%microsecs to secs
nDataP      = length(timeStNow);

%find time pairs at least 500 ms apart
reasonableTimePairs	= arrayfun(@(x)[x (find(timeStNow>timeStNow(x)+tStep,1))], ...
                                         (1:nDataP)', 'UniformOutput',false);
reasonableTimePairs(cellfun(@size,reasonableTimePairs,repmat({2},size(reasonableTimePairs))) < 2) = []; 
indxPairs           = cell2mat(reasonableTimePairs);
nTimePairs          = size(indxPairs,1);

%keep a timepair only if the positions of at least 3 sensors are known for
%that time interval
goodTimePairs	= arrayfun(@(x)[indxPairs(x,:) length(unique(rx_id(indxPairs(x,1):indxPairs(x,2)))) > 2], ...
                                         (1:nTimePairs)', 'UniformOutput',false);
goodTimePairs(cellfun(@size,goodTimePairs,repmat({2},size(goodTimePairs))) < 2) = []; 
finalIndxs      = cell2mat(goodTimePairs);
nIndxs          = size(finalIndxs, 1);

%for each pair of indexes containing at least 3 sensor positions,
%triangulate the center of the vehicle based on uss-center distances
allData         = str2double([timeStNow , dataTbl{:,1:18}]);
distMatx        = relUssPositions{:, :};
centerData      = [];

fBox = msgbox('Please wait, triangulating ego position...');
% fWb = waitbar(0,'%','Name','Triangulating ego position...');
%triangulate positions for all pairs of indices
%hfigTemp = figure('Visible','on');
%hold on;
for iIndx = 1:nIndxs
    
%     waitbar(iIndx/nIndxs,fWb,sprintf(['%3.2f %% ' num2str(nIndxs) ...
%                                         ' times stamps done'], 100*iIndx/nIndxs));
    ind1            = finalIndxs(iIndx,1);
    ind2            = finalIndxs(iIndx,2);
    dataMatNow      = allData(ind1:ind2, :);
    rxes            = dataMatNow(:,3);
    [Rx,indRx,~]    = unique(rxes);
    
    %sort them
    temMTx = [indRx, Rx];
    temMTx = sortrows(temMTx);
    indRx = temMTx(:,1);
    Rx    = temMTx(:,2);
    ...
    Rx = Rx(1:3);
    %take first 3, enough for trilateration
    coordRx     = dataMatNow(indRx(1:3), 12:14);
    [C,~,ib]   = intersect(distMatx(:, 1), Rx);
    distInd     = sortrows([ib, C]);
    radii       = distMatx(distInd(:,2),2);
    ...
    % Estimated position:
    yEstim = temp1 / temp2;
    xEstim = (va - yEstim*(yc-yb)) / (xc-xb);
    
    egoTemp = [xEstim, yEstim];

    %data: timestamp of the last Rx position and center of ego
    tempData    = [dataMatNow(indRx(3), 1), egoTemp];
    centerData  = [centerData;tempData];
    
%     plot(coordRx(:,1), coordRx(:,2), '.r');
%     hold on;
%     plot(egoTemp(1), egoTemp(2), '.g');
%     
%     xlim([-4 10]);
%     ylim([-16 4]);
%     
...
delete(fBox);
%plot3(centerData(:,1), centerData(:,2),centerData(:,3),'.r');

end                                                       

%==========================================================================
function [figHan] = doDirectionStats(dataMatx, toColorScheme)
% input data matrix with 3 columns TO id, Rx id, Rx-Np direction
%to add statistics when usp data available

RxList      = unique(dataMatx(:,2));
nUSSs       = length(RxList);
ToList      = unique(dataMatx(:,1));

[m,n]       = ndgrid(ToList,RxList);
rxTO_Combos	= [m(:),n(:)];
nCombos     = size(rxTO_Combos,1);

%make matrix with Rx-TO direction for all each combination of Rx and TO
%this makes a cell holding all such matrices
dirSummCell  = arrayfun(@(x) [(ones(sum((dataMatx(:,1) == rxTO_Combos(x,1) & dataMatx(:,2) == rxTO_Combos(x,2))),1)) * rxTO_Combos(x,1) ...%To id 
                (ones(sum((dataMatx(:,1) == rxTO_Combos(x,1) & dataMatx(:,2) == rxTO_Combos(x,2))),1)) * rxTO_Combos(x,2) ... %Rx id
                dataMatx((dataMatx(:,1) == rxTO_Combos(x,1) & dataMatx(:,2) == rxTO_Combos(x,2)), 3)] ... % direction 
                       , (1:nCombos)', 'UniformOutput',false);

%plot data for each sensor: one subplot per sensor with color-coded TOs
dirSumMatx  = cell2mat(dirSummCell);
figHan = figure('Name','TO Direction', 'units','normalized', ...
                    'outerposition',[0.05 0.12  0.9 0.75], 'Visible', 'on');
%subpots for 12 sensors
for iUSS =1:nUSSs
    rxNow   = RxList(iUSS); %range: 1 to 12
    subplot(2, 6, rxNow);
    hold on;
    title(['USS ' num2str(rxNow)]);
    rxNow   = RxList(iUSS);
    rxMat   = dirSumMatx(dirSumMatx(:,2) == rxNow, :);
    if(isempty(rxMat))
        continue;
    end
    TOsNow  = unique(dirSumMatx(:,1));
    nTOs    = length(TOsNow);
    for iTo = 1:nTOs
        toNow = TOsNow(iTo);
        toMatx = rxMat(rxMat(:,1) == toNow, :);
        if(isempty(toMatx))
            continue;
        end
        nDots = size(toMatx, 1);
        colorNow = zeros(nDots,3) + toColorScheme(toNow+1, :);
        %show traffic objects in 2D (x,y)
        szDots              = zeros(nDots,1) +10;
        scatter((1:nDots)', toMatx(:,3), szDots, colorNow);
        ylim([-180, 180]);
        pbaspect([1 1 1]);
    end%for iTo
    hold off;
...
%==========================================================================
function [figHan] = doAmplitudeStats(ampMatx, colourSchemeTo)
%input: amplitude matrix whith columns: timestamps, aplitude, TOid, rxId,
%txId, LoF

%remove cross echoes
ampMatx               = str2double(ampMatx);
ampMatx(ampMatx(:,5) ~= ampMatx(:,4), :) = [];

RxList      = unique(ampMatx(:,4));
nUSSs       = length(RxList);
ToList      = unique(ampMatx(:,3));

[m,n]       = ndgrid(ToList,RxList);
rxTO_Combos	= [m(:),n(:)];
nCombos     = size(rxTO_Combos,1);

ampSummCell  = arrayfun(@(x) [(ones(sum((ampMatx(:,3) == rxTO_Combos(x,1) & ampMatx(:,4) == rxTO_Combos(x,2))),1)) * rxTO_Combos(x,1) ...%To id 
                (ones(sum((ampMatx(:,3) == rxTO_Combos(x,1) & ampMatx(:,4) == rxTO_Combos(x,2))),1)) * rxTO_Combos(x,2) ... %Rx id
                ampMatx((ampMatx(:,3) == rxTO_Combos(x,1) & ampMatx(:,4) == rxTO_Combos(x,2)), 2)] ... % amplitude 
                       , (1:nCombos)', 'UniformOutput',false);

%plot data for each sensor: one subplot per sensor with color-coded TOs
ampSumMatx  = cell2mat(ampSummCell);
figHan = figure('Name','Echo amplitude', 'units','normalized', ...
                    'outerposition',[0.05 0.12  0.9 0.75], 'Visible', 'on');
%subpots for 12 sensors
for iUSS =1:nUSSs
    rxNow   = RxList(iUSS); %range: 1 to 12
    ...
    title(['USS ' num2str(rxNow)]);
    rxNow   = RxList(iUSS);
    rxMat   = ampSumMatx(ampSumMatx(:,2) == rxNow, :);
    if(isempty(rxMat))
        continue;
    end
    TOsNow  = unique(ampSumMatx(:,1));
    nTOs    = length(TOsNow);
    for iTo = 1:nTOs
        toNow = TOsNow(iTo);
        toMatx = rxMat(rxMat(:,1) == toNow, :);
        ...
        nDots = size(toMatx, 1);
        colorNow = zeros(nDots,3) + colourSchemeTo(toNow+1, :);
        %show traffic objects in 2D (x,y)
        szDots              = zeros(nDots,1) +10;
        scatter((1:nDots)', toMatx(:,3), szDots, colorNow);
        ylim([0, 255]);
