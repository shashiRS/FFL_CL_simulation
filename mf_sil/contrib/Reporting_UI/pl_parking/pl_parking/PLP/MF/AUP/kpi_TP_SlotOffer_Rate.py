...
def check_collinearity(timestamp_data, tolerance=0.1, **kwargs):
    ...
    for timestamp, points in timestamp_data.items():
        if (
            ...
            # Get the two points with the smallest x values
            left_line_points = sorted_points[:2]
            point1 = left_line_points[0]
            point2 = left_line_points[1]
            ...
            sorted_points = sorted(points, key=lambda p: p["x"])
            # Get the two points with the smallest x values
            right_line_points = sorted_points[2:]
            point1 = right_line_points[0]
            point2 = right_line_points[1]
    ...
    def __init__(self):
        ...
        self._properties = {
            ...
            self.Columns.sg_time: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.sSigHeader.uiTimeStamp",
                ".MF_TRJPLA_DATA.syncRef.m_sig_parkingBoxPort.uiTimeStamp",
            ],
            self.Columns.sg_parksm_core_status_stop_reason: [
            ...
            self.Columns.sg_parksm_core_status_state: [
                ".EM_DATA.EmPARRKSMCoreStatusPort.parksmCoreState_nu",
            ...
            self.Columns.sg_velocity: [".EM_DATA.EmEgoMotionPort.vel_mps", ".SI_DATA.m_egoMotionPort.vel_mps"],
            self.Columns.sg_selected_parking_box_id: [
                ".TRJPLA_DATA.TrjPlaSlotCtrlPort.selectedParkingBoxId_nu",
                ".MF_PARKSM_CORE_DATA.slotCtrlPort.selectedParkingBoxId_nu",
            ],
            self.Columns.sg_parking_box_1_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[0].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[0].parkingBoxID_nu",
            ],
            self.Columns.sg_parking_box_2_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[1].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[1].parkingBoxID_nu",
            ],
            self.Columns.sg_parking_box_3_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[2].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[2].parkingBoxID_nu",
            ],
            self.Columns.sg_parking_box_4_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[3].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[3].parkingBoxID_nu",
            ],
            self.Columns.sg_parking_box_5_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[4].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[4].parkingBoxID_nu",
            ],
            self.Columns.sg_parking_box_6_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[5].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[5].parkingBoxID_nu",
            ],
            self.Columns.sg_parking_box_7_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[6].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[6].parkingBoxID_nu",
            ],
            self.Columns.sg_parking_box_8_id: [
                ".TRJPLA_DATA.TrjPlaParkingBoxPort.parkingBoxes[7].parkingBoxID_nu",
                ".MF_TRJPLA_DATA.taposdDebugPort.poseBoxDataset.data[7].parkingBoxID_nu",
            ],
        }
        iteration_tuples = []
        for i in range(8):  # 8
...
class GenericTestStep(TestStep):
    ...
    def process(self, collinear_timestamp):
        ...
        try:
            ...
            image1_filename = r"TP-SlotOffer-imgs\next_to_parkingbox.png"
            image2_filename = r"TP-SlotOffer-imgs\fully_passed_parkingbox.png"
            ...
            if file_path.endswith(".rrec"):
                ...
                if json_files:
                    ...
                    with open(json_path) as file:
                        ...
                        # Navigate through the JSON structure to extract the key ending with .uiTimeStamp
                        parking_started = file_data.get("ParkingStarted", [])
                        if parking_started:
                            timed_objs = parking_started[0].get("timedObjs", [])
                            if timed_objs:
                                # Look for the key that ends with ".uiTimeStamp"
                                time_obj_string_timestamp = None
                                for key in timed_objs[0].keys():
                                    if key.endswith(".uiTimeStamp"):
                                        time_obj_string_timestamp = key
                                        break

                                if time_obj_string_timestamp is not None:
                                    print(f"Time Object String Timestamp Key: {time_obj_string_timestamp}")
                                else:
                                    raise ValueError("No key ending with '.uiTimeStamp' found in timedObjs")
                            else:
                                raise ValueError("timedObjs not found in ParkingStarted")
                        else:
                            raise ValueError("ParkingStarted not found in JSON file")
                else:
                    raise FileNotFoundError(f"No JSON file containing '{identifier}' found in {directory}")
            else:
                raise ValueError("The file does not have a '.rrec' extension")

            time = df[Signals.Columns.sg_time] / constants.GeneralConstants.US_IN_S
            while int(time.iloc[0]) == 0:
            ...
            # filtered_df.to_excel("filtered_df_pt_slots.xlsx")
            ...
            if eval_cond[0]:
                    ...
                    for centers_signal, _ in centers_signal_dict.items():
                        for centers_gt, _ in gt_timestamp.items():
                            ...
                            distance = calculate_distance(
                                centers_signal, centers_gt
                            )  # Assuming a function to calculate distance
                            if distance < min_distance:
                                min_distance = distance
                                targeted_selected_slot = targeted_slot
                ...
                for entry in file_data["VehicleLocal"]:
                    for timesdobjs in entry["timedObjs"]:
                        ...
                        if len(timesdobjs.get("parkingBoxes", [])) >= targeted_selected_slot - 1:
                            selected_parking_box = timesdobjs["parkingBoxes"][targeted_selected_slot - 1][
                                "slotCoordinates_m"
                            ]
                ...
                if "NextTo" in collinear_timestamp:
                    ...
                    image_path = os.path.join(image_base_path, image1_filename)
                    scenario = "NEXT TO PARKING BOX"
                else:
                    first_collinear_timestamp = check_collinearity(local_parking_box_dict, rear=rear)
                    image_path = os.path.join(image_base_path, image2_filename)
                ...
                if first_collinear_timestamp:
                    if next(iter(result_dict), None) < first_collinear_timestamp:
                        ...
                        gt_slot_selected = list(gt_timestamp.items())[
                            targeted_selected_slot - 1
                        ]  # The only PB that is considered is the one calculated in steps ahead of this one
                        centers_gt, parking_box_gt = gt_slot_selected
                        ...
                        if overlap_percentage > SlotOffer.Thresholds.THRESHOLD_OVERLAP:
                            ...
                            print(
                                f"Regarding {file_path}, the percentage of the overlap was not greater thatn the threshold, the scenario being {scenario}"
                            )
                    else:
                        test_result = fc.FAIL
                        self.result.measured_result = FALSE
                    ...
                    "Result": {
                        ...
                        "1": (
                            " ".join(
                                f"The parking slot was successfully chosen prior to the \
                                    specified timestamp[{first_collinear_timestamp}({round(transform_timestamp(first_collinear_timestamp, epoch),4)}s)].".split()
                            )
                            if eval_cond[1]
                            else " ".join(
                        ...
                        "2": (
                            " ".join(
                                ...
                                threshold({SlotOffer.Thresholds.THRESHOLD_OVERLAP}%) at timestamp {first_collinear_timestamp}({round(transform_timestamp(next(iter(result_dict), None), epoch),4)}s).".split()
                            )
            ...
            fig_img.update_layout(
                ...
                margin=dict(l=0, r=0, t=30, b=0),  # Adjusted top margin for the title
                title=dict(
                    text=f"Test scenario: {scenario}",
            ...
            fig1.add_trace(
                go.Scatter(
                    x=time.tolist(),
                    # x=(df[Signals.Columns.sg_time]/ constants.GeneralConstants.US_IN_S).values.tolist(),
            ...
            fig1.add_trace(
                go.Scatter(
                    x=time.tolist(),
                    # x=(df[Signals.Columns.sg_time]/ constants.GeneralConstants.US_IN_S).values.tolist(),
                    y=df[Signals.Columns.sg_parksm_core_status_state].values.tolist(),
                    mode="lines",
