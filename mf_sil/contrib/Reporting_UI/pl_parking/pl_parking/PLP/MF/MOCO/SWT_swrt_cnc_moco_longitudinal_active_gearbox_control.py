"""moco longitudinal active gearbox control"""
...
ALIAS = "SWT_swrt_cnc_moco_longitudinal_active_gearbox_control"
...
class Step1(TestStep):
    """moco longitudinal active gearbox control test setup"""
        ...
        try:
            ...
            if (df["activateLoCtrl"] == 1).any():
                ...
                if percent_match == 100:
                    ...
                    test_result = fc.PASS
                    eval_text = " ".join(
                        f"The component has set gearboxCtrlRequestPort.gearboxCtrlRequest_nu to true, "
                    ...
                    test_result = fc.FAIL
                    eval_text = " ".join(
                        f"The component has not set gearboxCtrlRequestPort.gearboxCtrlRequest_nu to true, "
                        f" Conditions: {test_result}.".split()
                    )

                self.result.measured_result = observed_la_dmc_ctrl_request_nu
                eval_0 = " ".join(
                    "Ensure that the component maintains the activation of gear "
                    "box control as long as the input request signal is active.".split()
                )
...
@verifies("1407043")
@testcase_definition(
    name="MOCO Longitudinal active gearbox control",
    group="gearbox",
    description="Ensure that the component maintains the activation of gear box control as long as the input request signal is active.",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class SWT_swrt_cnc_moco_longitudinal_active_gearbox_control(TestCase):
    """Example test case."""
