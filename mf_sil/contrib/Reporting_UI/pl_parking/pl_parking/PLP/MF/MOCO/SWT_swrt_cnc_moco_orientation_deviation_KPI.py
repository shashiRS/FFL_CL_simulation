"""moco_orientation_deviation_KPI"""
...
ALIAS = "SWT_swrt_cnc_moco_orientation_deviation_KPI"
...
@teststep_definition(
    ...
    name="KPI",
    description=" ",
    expected_result=f">= {constants.MoCo.Parameter.AP_C_KPI_SUCCESS_RATE_MAX_YAW_ERROR_ACCEPT_PERCENT} %",
)
@register_signals(ALIAS, MoCoSignals)
class Step1(TestStep):
    """moco orientation deviation KPI test setup"""
            ...
            if not df_filtered.empty:
                ...
                if assertion_dict:
                    ...
                    eval_text = " ".join(
                        ...
                        f"AP_C_KPI_FAIL_MAX_YAW_ERROR_DEG({constants.MoCo.Parameter.AP_C_KPI_FAIL_MAX_YAW_ERROR_DEG})"
                        f"Conditions: {test_result}.".split()
                    )
                    eval_0 = " ".join(
                        " MF Control shall ensure, that the absolute Orientation deviation from path stays as low as possible.".split()
                    )
...
def get_plot_signals(df):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=[
                constants.MoCo.Parameter.AP_C_KPI_FAIL_MAX_YAW_ERROR_DEG,
                constants.MoCo.Parameter.AP_C_KPI_FAIL_MAX_YAW_ERROR_DEG,
            ],
            mode="lines",
            name="AP_C_KPI_FAIL_MAX_YAW_ERROR_DEG",
            visible="legendonly",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=[df.index.values.min(), df.index.values.max()],
            y=[
                constants.MoCo.Parameter.AP_C_KPI_PASS_MAX_YAW_ERROR_DEG,
                constants.MoCo.Parameter.AP_C_KPI_PASS_MAX_YAW_ERROR_DEG,
            ],
            mode="lines",
            name="AP_C_KPI_PASS_MAX_YAW_ERROR_DEG",
            visible="legendonly",
        )
    )
...
@verifies("1735265")
@testcase_definition(
    name="MOCO Orientation Deviation KPI",
    group="yaw error",
    description="MF Control shall ensure, that the absolute Orientation deviation from path stays as low as possible",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class SWT_swrt_cnc_moco_orientation_deviation_KPI(TestCase):
    """Example test case."""
