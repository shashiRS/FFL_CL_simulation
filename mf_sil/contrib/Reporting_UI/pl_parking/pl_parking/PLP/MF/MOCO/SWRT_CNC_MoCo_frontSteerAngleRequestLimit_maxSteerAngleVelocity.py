"""MoCo frontSteerAngleRequestLimit maxSteerAngleVelocity"""
...
ALIAS = "SWRT_CNC_MoCo_frontSteerAngleRequestLimit_maxSteerAngleVelocity"
...
class Step1(TestStep):
    """MoCo frontSteerAngleRequestLimit maxSteerAngleVelocity test setup"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            "Calculating front steer angle request velocity in radian/second"
            ...
            "Calculating the gradient of first and next Ts in steerangleRequest"
            try:
                steeranglegrad = (
                    laDMCCtrlRequest_nu_present.iloc[0]["steerAngFrontAxle_rad"]
                    - laDMCCtrlRequest_nu_present.iloc[1]["frontSteerAngReq_rad_at_wheels"]
                )
            except Exception:
            ...
            if not df_filtered.empty:
                ...
                "TestStep"
                "Check if front steer angle velocity is within AP_V_MAX_STEER_ANG_VEL_RADPS"
                passed = []
                first_iteration = 1
                for ts, car_pos_row in df_filtered.iterrows():
                    if first_iteration == 1:
                        front_steer_angle = steeranglegrad
                        df.at[ts, "frontSteerAngReq_rad_sec"] = steeranglegrad
                        first_iteration = 0
                    else:
                        front_steer_angle = car_pos_row["frontSteerAngReq_rad_sec"]

                    if (abs(front_steer_angle)) < constants.MoCo.Parameter.AP_V_MAX_STEER_ANG_VEL_RADPS:
                        passed.append(True)
                    else:
                        passed.append(False)
                if all(passed):
                ...
                eval_0 = " ".join(
                    ...
                    f"Front steer angle velocity should be within "
                    f"AP_V_MAX_STEER_ANG_VEL_RADPS({constants.MoCo.Parameter.AP_V_MAX_STEER_ANG_VEL_RADPS}).".split()
                )
                ...
                eval_0 = " ".join(
                    ...
                    f" and Front steer angle velocity should be within "
                    f"AP_V_MAX_STEER_ANG_VEL_RADPS({constants.MoCo.Parameter.AP_V_MAX_STEER_ANG_VEL_RADPS}).".split()
                )
                # Set table dataframe
                signal_summary = pd.DataFrame(
...
def get_plot_signals(df):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=[
                constants.MoCo.Parameter.AP_V_MAX_STEER_ANG_VEL_RADPS,
                constants.MoCo.Parameter.AP_V_MAX_STEER_ANG_VEL_RADPS,
            ],
            mode="lines",
            name="AP_V_MAX_STEER_ANG_VEL_RADPS",
            visible="legendonly",
        )
    )
...
@verifies("1407000")
@testcase_definition(
    name="MoCo frontSteerAngleRequestLimit maxSteerAngleVelocity",
    group="lateral error",
    description="The component shall limit the front steer angle request velocity is within AP_V_MAX_STEER_ANG_VEL_RADPS.",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class SWRT_CNC_MoCo_frontSteerAngleRequestLimit_maxSteerAngleVelocity(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    debug(
        SWRT_CNC_MoCo_frontSteerAngleRequestLimit_maxSteerAngleVelocity,
        *test_bsigs,
        temp_dir=temp_dir,
