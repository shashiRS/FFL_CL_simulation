"""moco orientation path control status port"""
...
ALIAS = "SWT_swrt_cnc_moco_orientation_path_control_status_port"
...
class Step1(TestStep):
    """moco orientation path control status port test setup"""
    ...
    def process(self, **kwargs):
        ...
        try:
            ...
            if not df_filtered.empty:
                "TestStep"
                "Evaluate orientation path control status"
                for _, car_pos_row in df_filtered.iterrows():
                    orientation_deviation = car_pos_row["orientationError_rad"]
                    if (abs(orientation_deviation) > constants.MoCo.Parameter.AP_C_PC_FAIL_MAX_YAW_ERROR_RAD
                ...
                if all(passed):
                    ...
                    eval_text = " ".join(
                        ...
                        f"absolute orientation deviation from path is larger than AP_C_PC_FAIL_MAX_YAW_ERROR_RAD({constants.MoCo.Parameter.AP_C_PC_FAIL_MAX_YAW_ERROR_RAD})"
                        f"and the component output is (mfControlStatusPort.lateralPathControlFailed_nu=False)"
                        f"Conditions: {test_result}.".split()
                    )

                eval_0 = " ".join(
                    f" Lateral control request sent and if absolute orientation deviation from path is larger than "
                    f" AP_C_PC_FAIL_MAX_YAW_ERROR_RAD ({constants.MoCo.Parameter.AP_C_PC_FAIL_MAX_YAW_ERROR_RAD}) the component shall output (mfControlStatusPort.lateralPathControlFailed_nu=true).".split()
                )
                ...
                self.result.measured_result = NAN
                eval_0 = " ".join(
                    f" Lateral control request sent and if absolute orientation deviation from path is larger than AP_C_PC_FAIL_MAX_YAW_ERROR_RAD ({constants.MoCo.Parameter.AP_C_PC_FAIL_MAX_YAW_ERROR_RAD})the component shall output (mfControlStatusPort.lateralPathControlFailed_nu=true).".split()
                )
                # Set table dataframe
                signal_summary = pd.DataFrame(
                    {
...
@verifies("1407026")
@testcase_definition(
    name="MOCO orientation path control status port",
    group="orientation Path Control",
    description="Ensure that if vehicle in standstill and the orientation deviation from path is larger than {AP_C_PC_FAIL_MAX_YAW_ERROR_RAD} the component shall output (mfControlStatusPort.lateralPathControlFailed_nu=true)",
)
@register_inputs("/Playground_2/TSF-Debug")
# @register_inputs("/TSF_DEBUG/")
class SWT_swrt_cnc_moco_orientation_path_control_status_port(TestCase):
    """Example test case."""
...
def main(data_folder: Path, temp_dir: Path = None, open_explorer=True):
    ...
    execute the test (e.g. a BSIG file) and report th.e result.
    ...
    debug(
        SWT_swrt_cnc_moco_orientation_path_control_status_port,
        *test_bsigs,
        temp_dir=temp_dir,
