"""moco longitudinal inactive control"""
...
ALIAS = "SWT_swrt_cnc_moco_longitudinal_inactive_control"
...
class Step1(TestStep):
    """moco longitudinal inactive control test setup"""
        ...
        try:
            ...
            if (df["activateLoCtrl"] == 0).any():
                ...
                if percent_match == 100:
                    ...
                    test_result = fc.PASS
                    eval_text = " ".join(
                        f" The component has not set loDMCCtrlRequestPort.loDMCCtrlRequest_nu to false"
                    ...
                    test_result = fc.FAIL
                    eval_text = " ".join(
                        f" The component has set loDMCCtrlRequestPort.loDMCCtrlRequest_nu to false"
                        f" Conditions: {test_result}.".split()
                    )

                self.result.measured_result = observed_la_dmc_ctrl_request_nu
                eval_0 = " ".join(
                    "Ensure that the component does not activate longitudinal control when the input request is set to false.".split()
                )
                ...
                eval_text = "Precondition not met"
                self.result.measured_result = NAN
