...
try:
    ...
    header_file = os.path.join('./input', 'report', 'header_L3.txt')
    with open(header_file, "r"):
        pass
except:
    ...
    header_file = os.path.join(os.path.dirname(__file__), 'header_L3.txt')
...
class cHTMLReportTemplate(cHTMLReportTemplateBase):
    ...
    def CreateTableBegin(self, caption="", tableCount=0, sortable=True, width=None):
        ...
        tableBegin += '<br><div class="scroll"><table id="table{tableCount}" class="niceTable" width="{width}" border="0" cellpadding="0" cellspacing="0"><caption align="bottom" style="color: #FFFFFF">{tableCaption}</caption>'.format(
    ...
    def CreateImageGraphScript(self):
        ...
        for Id in self._graphId:
            jsFileString += (
                ...
                'itemclick: myEventHandler{graphID}\n'
                ...
                '$(\'#carmakerImage{graphID}\').html(eventData);\n'
                'var eventData2 = \'<div><br><br><br><img src = \' + e.dataPoint.B + \' position=average caretPadding=6 height=300 width=400/>\' + \'</div>\';\nchart{graphID}.render();\n'
                '$(\'#birdEyeImage{graphID}\').html(eventData2);\n'
                '}'
