// Attention, this file is generated by Cobolt from template: D:\.bbs_conan\c029a0\1\codegen\templates\types\enum.h.template!

#ifdef ECO_C_TYPES_USED
  #ifndef ECO_ALLOW_INTERFACE_MIXING
    #error eco C and C++ types mixed
  #endif
#endif
#ifdef ECO_CPP14_TYPES_USED
  #ifndef ECO_ALLOW_INTERFACE_MIXING
    #error eco C++ and C++14 types mixed
  #endif
#endif
#define ECO_CPP_TYPES_USED

#ifndef AP_COMMON_VEHICLE_BOUNDING_SHAPE_CORNERS_H_
#define AP_COMMON_VEHICLE_BOUNDING_SHAPE_CORNERS_H_

#include "Platform_Types.h"
#include "eco/deprecation.h"

// PRQA S 2180 ++
/* date: 2020-03-31, reviewer: PR reviewers of https://github-am.geo.conti.de/ADAS/eco/pull/1100, CCBIssueId: https://jira-adas.zone2.agileci.conti.de/browse/SEP-1137, reason: implicit construction is desired here */

namespace ap_common
{
  /// \cond HIDDEN_ENUMS
  class VehicleBoundingShapeCorners
  {
  public:
    typedef uint8 underlying_type;
#ifdef _MSC_VER
    enum do_not_use : underlying_type
#else
    enum do_not_use
#endif
    {
      FRONT_LEFT_INNER = 0U,
      FRONT_LEFT_OUTER = 1U,
      AXIS_LEFT = 2U,
      REAR_LEFT_OUTER = 3U,
      REAR_LEFT_INNER = 4U,
      REAR_RIGHT_INNER = 5U,
      REAR_RIGHT_OUTER = 6U,
      AXIS_RIGHT = 7U,
      FRONT_RIGHT_OUTER = 8U,
      FRONT_RIGHT_INNER = 9U,
      NUM_RELEVANT_CORNERS = 10U
    };

    VehicleBoundingShapeCorners() :
      value(static_cast< underlying_type >(FRONT_LEFT_INNER))
    {
    }

    VehicleBoundingShapeCorners(const do_not_use v) :
      value(static_cast<underlying_type >(v))
    {
    }

    VehicleBoundingShapeCorners(const VehicleBoundingShapeCorners& v) :
      value(static_cast<underlying_type >(v))
    {
    }

    DEPRECATED_BEGIN DEPRECATED_END VehicleBoundingShapeCorners(const underlying_type v) :
      value(v)
    {
    }

    operator underlying_type() const
    {
      return value;
    }

    VehicleBoundingShapeCorners& operator|=(const do_not_use v)
    {
      value |= static_cast< underlying_type >(v);
      return *this;
    }

    DEPRECATED(VehicleBoundingShapeCorners&) operator|=(const underlying_type v)
    {
      value |= v;
      return *this;
    }

    VehicleBoundingShapeCorners& operator=(const do_not_use v)
    {
      value = static_cast< underlying_type >(v);
      return *this;
    }

    VehicleBoundingShapeCorners& operator=(const VehicleBoundingShapeCorners& v)
    {
      value = static_cast< underlying_type >(v);
      return *this;
    }

    DEPRECATED(VehicleBoundingShapeCorners&) operator=(const underlying_type v)
    {
      value = v;
      return *this;
    }

    template < typename T >
    bool operator==(const T v) const
    {
      return value == static_cast< underlying_type >(v);
    }
    /// \endcond

  private:
    underlying_type value;
  };
} // namespace ap_common

// PRQA S 2180 --

#endif // AP_COMMON_VEHICLE_BOUNDING_SHAPE_CORNERS_H_
