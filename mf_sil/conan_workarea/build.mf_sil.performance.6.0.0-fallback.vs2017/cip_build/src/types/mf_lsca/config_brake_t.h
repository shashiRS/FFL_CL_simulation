...
#ifndef MF_LSCA_CONFIG_BRAKE_T_H_
#define MF_LSCA_CONFIG_BRAKE_T_H_
...
namespace mf_lsca
{
  ...
  inline ::mf_lsca::configBrake_t createconfigBrake_t()
  {
    configBrake_t m;
    (void)::eco::memset(&m, 0U, sizeof(configBrake_t));
    m.brakeModel = createbrakeSet_t();
    {
      const uint64 arraysize = (sizeof(m.bodyAdjustment) / sizeof(m.bodyAdjustment[0]));
      for(uint64 i = 0U; i < arraysize; ++i)
      {
        m.bodyAdjustment[i] = ::cml::createVec2Df_POD();
      }
    }
    {
      const uint64 arraysize = (sizeof(m.protectedMirrorShapeLeft) / sizeof(m.protectedMirrorShapeLeft[0]));
      for(uint64 i = 0U; i < arraysize; ++i)
      {
        m.protectedMirrorShapeLeft[i] = ::cml::createVec2Df_POD();
      }
    }
    {
      const uint64 arraysize = (sizeof(m.protectedMirrorShapeRight) / sizeof(m.protectedMirrorShapeRight[0]));
      for(uint64 i = 0U; i < arraysize; ++i)
      {
        m.protectedMirrorShapeRight[i] = ::cml::createVec2Df_POD();
      }
    }
    return m;
  }
...
namespace eco
{
  ...
  template<>
  inline ::mf_lsca::configBrake_t create_default()
  {
      return ::mf_lsca::createconfigBrake_t();
  }
}


#endif // MF_LSCA_CONFIG_BRAKE_T_H_
