// Attention, this file is generated by Cobolt from template: D:\.bbs_conan\c029a0\1\codegen\templates\types\enum.h.template!

#ifdef ECO_C_TYPES_USED
  #ifndef ECO_ALLOW_INTERFACE_MIXING
    #error eco C and C++ types mixed
  #endif
#endif
#ifdef ECO_CPP14_TYPES_USED
  #ifndef ECO_ALLOW_INTERFACE_MIXING
    #error eco C++ and C++14 types mixed
  #endif
#endif
#define ECO_CPP_TYPES_USED

#ifndef AP_LODMC_MANEUVERING_FINISHED_H_
#define AP_LODMC_MANEUVERING_FINISHED_H_

#include "Platform_Types.h"
#include "eco/deprecation.h"

// PRQA S 2180 ++
/* date: 2020-03-31, reviewer: PR reviewers of https://github-am.geo.conti.de/ADAS/eco/pull/1100, CCBIssueId: https://jira-adas.zone2.agileci.conti.de/browse/SEP-1137, reason: implicit construction is desired here */

namespace ap_lodmc
{
  /// \cond HIDDEN_ENUMS
  ///Status of the maneuvering.
  class maneuveringFinished
  {
  public:
    typedef uint8 underlying_type;
#ifdef _MSC_VER
    enum do_not_use : underlying_type
#else
    enum do_not_use
#endif
    {
      MANEUVERING_NOT_STARTED = 0U,
      MANEUVERING_IN_PROGRESS = 1U,
      MANEUVERING_FINISHED = 2U,
      MANEUVERING_SATURATED = 3U,
      MANEUVERING_FAULT = 4U,
      RESERVED = 5U
    };

    maneuveringFinished() :
      value(static_cast< underlying_type >(MANEUVERING_NOT_STARTED))
    {
    }

    maneuveringFinished(const do_not_use v) :
      value(static_cast<underlying_type >(v))
    {
    }

    maneuveringFinished(const maneuveringFinished& v) :
      value(static_cast<underlying_type >(v))
    {
    }

    DEPRECATED_BEGIN DEPRECATED_END maneuveringFinished(const underlying_type v) :
      value(v)
    {
    }

    operator underlying_type() const
    {
      return value;
    }

    maneuveringFinished& operator|=(const do_not_use v)
    {
      value |= static_cast< underlying_type >(v);
      return *this;
    }

    DEPRECATED(maneuveringFinished&) operator|=(const underlying_type v)
    {
      value |= v;
      return *this;
    }

    maneuveringFinished& operator=(const do_not_use v)
    {
      value = static_cast< underlying_type >(v);
      return *this;
    }

    maneuveringFinished& operator=(const maneuveringFinished& v)
    {
      value = static_cast< underlying_type >(v);
      return *this;
    }

    DEPRECATED(maneuveringFinished&) operator=(const underlying_type v)
    {
      value = v;
      return *this;
    }

    template < typename T >
    bool operator==(const T v) const
    {
      return value == static_cast< underlying_type >(v);
    }
    /// \endcond

  private:
    underlying_type value;
  };
} // namespace ap_lodmc

// PRQA S 2180 --

#endif // AP_LODMC_MANEUVERING_FINISHED_H_
