...
namespace si
{
  ...
  inline ::si::HighPlotData createHighPlotData()
  {
    HighPlotData m;
    (void)::eco::memset(&m, 0U, sizeof(HighPlotData));
    m.delimiterZones = createDelimiterZonesSerializable();
    m.surroundingLinesPerSlot = createSlotSurroundingLinesSerializable();
    m.virtualLinesInfo = createVirtualLineInfoSerializable();
    m.triangleExpansionRois = createTriangleExpansionSerializable();
    m.sideExpansionRois = createQuadrilateralExpansionSerializable();
    m.cnnBlowUpLimits = createQuadrilateralSerializableArray();
    m.slotBlowUpLimits = createQuadrilateralSerializableArray();
    m.rectSlots = createQuadrilateralSerializableArray();
    m.clusterZones = createHighClusterZoneSerializable();
    m.cnnBasedParkingSlots = createCnnBasedParkingSlotSerializable();
    return m;
  }
...
namespace eco
{
  ...
  template<>
  inline ::si::HighPlotData create_default()
  {
      return ::si::createHighPlotData();
  }
}
