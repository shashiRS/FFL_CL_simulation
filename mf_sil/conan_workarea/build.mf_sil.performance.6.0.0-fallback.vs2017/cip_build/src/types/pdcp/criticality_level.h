// Attention, this file is generated by Cobolt from template: D:\.bbs_conan\c029a0\1\codegen\templates\types\enum.h.template!

#ifdef ECO_C_TYPES_USED
  #ifndef ECO_ALLOW_INTERFACE_MIXING
    #error eco C and C++ types mixed
  #endif
#endif
#ifdef ECO_CPP14_TYPES_USED
  #ifndef ECO_ALLOW_INTERFACE_MIXING
    #error eco C++ and C++14 types mixed
  #endif
#endif
#define ECO_CPP_TYPES_USED

#ifndef PDCP_CRITICALITY_LEVEL_H_
#define PDCP_CRITICALITY_LEVEL_H_

#include "Platform_Types.h"
#include "eco/deprecation.h"

// PRQA S 2180 ++
/* date: 2020-03-31, reviewer: PR reviewers of https://github-am.geo.conti.de/ADAS/eco/pull/1100, CCBIssueId: https://jira-adas.zone2.agileci.conti.de/browse/SEP-1137, reason: implicit construction is desired here */

namespace pdcp
{
  /// \cond HIDDEN_ENUMS
  ///Criticality level for this sector (the higher the criticality, the closer the obstacle is to the car).
  class CriticalityLevel
  {
  public:
    typedef uint8 underlying_type;
#ifdef _MSC_VER
    enum do_not_use : underlying_type
#else
    enum do_not_use
#endif
    {
      PDC_CRIT_LVL_OUTSIDE = 0U,
      PDC_CRIT_LVL_GREEN = 1U,
      PDC_CRIT_LVL_YELLOW = 2U,
      PDC_CRIT_LVL_RED = 3U
    };

    CriticalityLevel() :
      value(static_cast< underlying_type >(PDC_CRIT_LVL_OUTSIDE))
    {
    }

    CriticalityLevel(const do_not_use v) :
      value(static_cast<underlying_type >(v))
    {
    }

    CriticalityLevel(const CriticalityLevel& v) :
      value(static_cast<underlying_type >(v))
    {
    }

    DEPRECATED_BEGIN DEPRECATED_END CriticalityLevel(const underlying_type v) :
      value(v)
    {
    }

    operator underlying_type() const
    {
      return value;
    }

    CriticalityLevel& operator|=(const do_not_use v)
    {
      value |= static_cast< underlying_type >(v);
      return *this;
    }

    DEPRECATED(CriticalityLevel&) operator|=(const underlying_type v)
    {
      value |= v;
      return *this;
    }

    CriticalityLevel& operator=(const do_not_use v)
    {
      value = static_cast< underlying_type >(v);
      return *this;
    }

    CriticalityLevel& operator=(const CriticalityLevel& v)
    {
      value = static_cast< underlying_type >(v);
      return *this;
    }

    DEPRECATED(CriticalityLevel&) operator=(const underlying_type v)
    {
      value = v;
      return *this;
    }

    template < typename T >
    bool operator==(const T v) const
    {
      return value == static_cast< underlying_type >(v);
    }
    /// \endcond

  private:
    underlying_type value;
  };
} // namespace pdcp

// PRQA S 2180 --

#endif // PDCP_CRITICALITY_LEVEL_H_
